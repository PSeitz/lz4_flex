...
pub fn decompress_into(input: &[u8], output: &mut Vec<u8>, dict: &Dict) -> Result<(), DecompressError> {
    ...
    loop {
        ...
        if does_token_fit(token) && is_safe_distance(input_pos, end_pos_check) {
            ...
            // if match_length <= 16 {
            //     let mut dst = [0u8; 16];
            //     dst.clone_from_slice(&dict_data[offset..offset + 16]);
            //     output.extend_from_slice(&dst);
            // } else {
            //     copy_24(output, offset)
            // }
            // let mut dst = [0u8; 32];
            // dst.clone_from_slice(&dict_data[offset..offset + 32]);
            // output.extend_from_slice(&dst);
            // output.truncate(old_len + match_length);

            output.extend_from_slice(&dict_data[offset..offset + match_length]);
        ...
        duplicate_slice(output, dict_data, offset, match_length)?;
    }
    Ok(())
}
...
pub fn duplicate_slice(
    ...
    // let old_len = output.len();
    // let mut dst = [0u8; 16];
    // for i in (offset..offset + match_length).step_by(16) {
    //     dst.clone_from_slice(&dict_data[i..i + 16]);
    //     output.extend_from_slice(&dst);
    // }
    // output.truncate(old_len + match_length);
    Ok(())
