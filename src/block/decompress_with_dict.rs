...
#[inline]
pub fn decompress_into(input: &[u8], output: &mut Vec<u8>, dict: &Dict) -> Result<(), DecompressError> {
    // Decode into our vector.
    let mut input_pos = 0;
    let mut output_ptr = output.as_mut_ptr();
    let dict_ptr = dict.get_data().as_ptr();
    ...
    loop {
        ...
        if does_token_fit(token) && is_safe_distance(input_pos, end_pos_check) {
            ...
            let start_ptr = unsafe { dict_ptr.add(offset as usize) };
            #[cfg(feature = "checked-decode")]
            {
                // TODO if dict.data is always at least u16::MAX big, we can remove the check
            }

            let match_length = (4 + (token & 0xF)) as usize;
            // Write the duplicate segment to the output buffer from the dict data

            // TODO dict.data needs to be u16::MAX + 32 to avoid out of bounds access
            unsafe{
                core::ptr::copy_nonoverlapping(start_ptr, output_ptr, 32);
        ...
        unsafe {
            wild_copy_from_src_16(start_ptr, output_ptr, match_length);
            output_ptr = output_ptr.add(match_length);
        }
    }
    Ok(())
...
#[inline]
pub fn decompress_size_prepended(input: &[u8], dict: &Dict) -> Result<Vec<u8>, DecompressError> {
    ...
    decompress_into(input, &mut vec, dict)?;
...
#[inline]
pub fn decompress_with_dict(input: &[u8], uncompressed_size: usize, dict: &Dict) -> Result<Vec<u8>, DecompressError> {
    // Allocate a vector to contain the decompressed stream. we may wildcopy out of bounds, so the vector needs to have ad additional BLOCK_COPY_SIZE capacity
    let mut vec = Vec::with_capacity(uncompressed_size + 4 + BLOCK_COPY_SIZE);
    unsafe {
        vec.set_len(uncompressed_size);
